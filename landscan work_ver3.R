...
## mask(cut out) ocean parts by uploading Peru polygon
peru = readOGR(dsn="./peruBorder")
smth = mask(smth, peru)

## save it as a polygon because arcGIS
tmpVal = rasterToPolygons(smth, dissolve=FALSE)
#writeOGR(obj = tmpVal, dsn = "PeruLandScan",layer="peru",driver="ESRI Shapefile",driver="ESRI Shapefile",driver="ESRI Shapefile",overwrite_layer=TRUE, delete_dsn=TRUE)
...
### !!NOTE!! In GIS software, they still need to be run through multipart to singlepart!
coreFringesPoly = rasterToPolygons(coreFringes, dissolve=TRUE)
coreFringesPoly = coreFringesPoly[coreFringesPoly>0]
coreFringesPoly = disaggregate(coreFringesPoly)
writeOGR(obj = coreFringesPoly, dsn = "coreFringes",layer="coreFringe",driver="ESRI Shapefile")

## settlements are still wrong. They include fringes...that isn't right
settlementPoly = rasterToPolygons(settlementAreas, dissolve=TRUE)
settlementPoly = settlementPoly[settlementPoly>0]
...
writeOGR(obj = tmpVal, dsn = "coreAreasLandScan",layer="coreAreas",driver="ESRI Shapefile")
...
writeOGR(obj = tmpVal, dsn = "coreFringeLandScan",layer="coreFringes",driver="ESRI Shapefile")
...
writeOGR(obj = tmpVal, dsn = "settlementLandScan",layer="settlements",driver="ESRI Shapefile")
